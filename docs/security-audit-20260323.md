# Security Audit — miniature-guacamole / TheEngOrg Enterprise
...
**Finding:** `mg-dev-key` is a shell script in the installer that creates a local enterprise session file (`~/.claude/enterprise-session.json`) with:
...
User-agent: *
Allow: /
Sitemap: https://theengorg.wontonwebworks.com/sitemap.xml
```
...
### LOW — `mg-dev-key` writes to `enterprise-session.json` without `umask` protection (race condition)
...
If a user is made a Wonton platform admin for any reason, they automatically gain the ability to issue and revoke enterprise licenses for all MG organizations. This is likely the intended behavior (Wonton admins = TheEngOrg admins), but it should be an explicit design decision documented in the code, not an implicit coupling.
...
**SEVERITY:** INFO
**LOCATION:** `~/.claude/enterprise-session.json`
...
### INFO — `LICENSE.enterprise` contains no sensitive information
