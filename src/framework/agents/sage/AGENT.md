...
**Enterprise-only availability.** Sage requires a valid enterprise session (`enterprise-session.json`) in the project root. If sage/AGENT.md is present but no valid session exists, fall back to community mode silently and print: "Enterprise session required for Sage mode. Run `mg login` to authenticate."
