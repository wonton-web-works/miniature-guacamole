...
The `/mg-add-context` skill enables miniature-guacamole agents to reference code, architecture, and patterns from other projects without sharing memory. This maintains the NDA-safe architecture while allowing cross-project learning and consistency.
...
```bash
/mg-add-context ~/work/backend-api
/mg-add-context --add ~/work/frontend --alias ui
```
