...
This directory contains the miniature-guacamole v2.x distribution with project-local installation.

## Overview

miniature-guacamole v2.x uses **project-local installation** - all framework files are installed to your project's `.claude/` directory, not to `~/.claude/`. This provides:
...
./mg-web-install.sh --version v2.0.0 /path/to/project
```
...
3. Install v2.x project-locally:
   ```bash
   cd /path/to/your/project
   /path/to/dist/miniature-guacamole/install.sh
   ```
