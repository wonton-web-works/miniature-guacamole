#!/usr/bin/env bats
# ============================================================================
# build-enterprise.bats - Tests for build-enterprise.sh script
# ============================================================================
...
setup() {
    ...
    cp -r "$PROJECT_ROOT"/{src,package.json,tsconfig.json,build-enterprise.sh} "$TEST_DIR/"
...
EOF
    ...
    SCRIPT_PATH="$TEST_DIR/build-enterprise.sh"
...
@test "build-enterprise.sh: enterprise/ directory doesn't exist" {
...
@test "build-enterprise.sh: enterprise/package.json is missing" {
...
@test "build-enterprise.sh: enterprise/package.json has invalid JSON" {
...
@test "build-enterprise.sh: enterprise/tsconfig.json is missing" {
...
@test "build-enterprise.sh: enterprise/tsconfig.json has invalid extends path" {
    cat > "$TEST_DIR/enterprise/tsconfig.json" << 'EOF'
{
...
@test "build-enterprise.sh: enterprise/src/ directory doesn't exist" {
...
@test "build-enterprise.sh: TypeScript compilation fails" {
...
@test "build-enterprise.sh: dist/ directory is read-only" {
...
@test "build-enterprise.sh: executed from wrong directory" {
...
@test "build-enterprise.sh: enterprise/src/ is empty" {
...
@test "build-enterprise.sh: enterprise/package.json with minimal fields" {
    cat > "$TEST_DIR/enterprise/package.json" << 'EOF'
{
  "name": "@mg/enterprise",
  "version": "1.0.0"
...
@test "build-enterprise.sh: enterprise has no subdirectories yet" {
...
@test "build-enterprise.sh: dist/ directory already exists" {
...
@test "build-enterprise.sh: enterprise/src/index.ts is empty file" {
...
@test "build-enterprise.sh: enterprise/ has node_modules/ subdirectory" {
...
@test "build-enterprise.sh: parallel build.sh execution" {
    skip "TODO: Test concurrent builds (requires OSS build.sh)"
}

@test "build-enterprise.sh: very large enterprise/ directory" {
...
@test "build-enterprise.sh: successful build with complete structure" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: dist output includes all enterprise files" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: dist output includes OSS framework files" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: creates tarball archive" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: creates zip archive" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: VERSION.json includes enterprise flag" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: preserves executable permissions" {
...
@test "build-enterprise.sh: TypeScript compilation produces .d.ts files" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: output size is reasonable" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: script displays progress output" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
...
@test "build-enterprise.sh: script reports build statistics" {
    run "$SCRIPT_PATH"
    [ "$status" -eq 0 ]
