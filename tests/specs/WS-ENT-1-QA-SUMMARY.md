...
1. **tests/scripts/build-ext.bats** (28 tests)
   - BATS tests for build-ext.sh script
...
**build-ext.bats**: 0 passed / 26 failed / 2 skipped
- All tests fail because build-ext.sh doesn't exist
...
| AC-ENT-1.2 | build-ext.sh creates dist | 18 tests | 99%+ |
...
### Build Validation
- Both build.sh and build-ext.sh succeed independently
...
- [x] BATS tests for build-ext.sh (28 tests)
...
2. **build-ext.sh should mirror build.sh**: Copy and modify to include enterprise/
...
**Potential gotchas**:
- Ensure build-ext.sh creates separate dist path (not overwriting OSS)
