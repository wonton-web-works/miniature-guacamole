...
### Test Tools
- **BATS** for build script testing (build-ext.sh, modified build.sh)
...
| AC-ENT-1.2 | build-ext.sh produces distribution including enterprise/ | 18 tests (BATS) |
...
24. build-ext.sh file exists
25. build-ext.sh is executable
26. build-ext.sh runs successfully
27. build-ext.sh creates dist/miniature-guacamole-enterprise/
28. build-ext.sh output includes enterprise/ directory
29. build-ext.sh creates VERSION.json with "enterprise": true
...
2. **build-ext.sh doesn't exist** → 28 build tests fail
...
1. `tests/scripts/build-ext.bats` - 28 tests
...
```bash
# Run BATS tests
bats tests/scripts/build-ext.bats
