/**
 * Unit Tests for Accessibility Review Skill
 *
 * Tests that accessibility-review skill exists with proper structure.
 * WS-11: Accessibility Review
 */
...
describe('Accessibility Review - WS-11', () => {
  describe('mg-accessibility-review SKILL.md', () => {
    const skillPath = path.join(SKILLS_DIR, 'mg-accessibility-review', 'SKILL.md');
    ...
    it('should have frontmatter with name', () => {
      ...
      expect(content).toMatch(/name:\s+mg-accessibility-review/);
    });
    ...
    it('should mention WCAG in content', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content).toMatch(/WCAG/);
    });

    it('should mention WCAG criteria format (e.g., 1.4.3, 2.1.1)', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content).toMatch(/\d+\.\d+\.\d+/);
    });

    it('should mention keyboard navigation', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content.toLowerCase()).toMatch(/keyboard.*navigation|keyboard.*nav/);
    });

    it('should mention screen reader', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content.toLowerCase()).toMatch(/screen.*reader/);
    });

    it('should mention color contrast', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content.toLowerCase()).toMatch(/color.*contrast|contrast.*ratio/);
    });

    it('should mention ARIA', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content).toMatch(/ARIA/);
    });

    it('should mention semantic HTML', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content.toLowerCase()).toMatch(/semantic.*html/);
    });

    it('should mention focus management', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content.toLowerCase()).toMatch(/focus.*manag/);
    });

    it('should mention spawning qa or design', () => {
      const content = fs.readFileSync(skillPath, 'utf-8');
      expect(content.toLowerCase()).toMatch(/spawn.*qa|spawn.*design/);
    });
  });
